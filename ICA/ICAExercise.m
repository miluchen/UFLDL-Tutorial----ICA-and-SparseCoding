...
fprintf('%11s%16s%10s%16s\n','Iteration','Cost','alpha','linearDelta');
...
for iteration = 1:10000
    ...
    while 1
		...
		% using Gram-Schmidt method to transfer the row vectors of considerWeightMatrix to be orthonormal
		% the row number of considerWeightMatrix should be larger than the column number
		%for i = 1:size(considerWeightMatrix, 1);
		%	v = considerWeightMatrix(i,:);
		%	for j = 1:i-1;
		%		coef = considerWeightMatrix(j,:)*v';
		%		v = v - coef*considerWeightMatrix(j,:);
		%	end;
		%	considerWeightMatrix(i,:) = v/norm(v);
		%end;
		% or another method (using inverse of a matrix):
		% B = (A*A')^(-0.5)*A;
    ...
    fprintf('  %9d  %14.6f  %8.7g  %14.8f\n', iteration, newCost, alpha, linearDelta);
