...
delta2 = (W2'*delta3 + beta*(-sparsityParam./rrho + (1-sparsityParam)./(1-rrho))).*A2.*(1-A2);
W1grad = delta2*data'/m + lambda*W1;
b1grad = sum(delta2, 2)/m;

% compute the final cost
square_error = sum(sum((A3-data).^2))/(2*m);
weight_decay = lambda/2*(sum(sum(W1.^2))+sum(sum(W2.^2)));
sparse_penalty = beta*sum(sparsityParam*log(sparsityParam./rho) + (1-sparsityParam)*log((1-sparsityParam)./(1-rho)));
cost = square_error + weight_decay + sparse_penalty;
